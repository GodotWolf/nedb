<?php
	...
	function bonosPorCarnet($id_carnet){
		...
		$sql = "SELECT BONORECIBIDO.*, BONO.NOMBRE FROM BONORECIBIDO JOIN BONO ON (BONORECIBIDO.ID_BONO = BONO.ID) JOIN CARNET ON (CARNET.ID = BONORECIBIDO.ID_CARNET)";
	...
	function programasSocialesPorPersona($id_persona){
		...
		$sql = "SELECT PROGRAMAASIGNADO.*, PROGRAMASOCIAL.NOMBRE AS NOMBRE_PROGRAMA FROM PROGRAMAASIGNADO JOIN PROGRAMASOCIAL ON (PROGRAMAASIGNADO.ID_PROGRAMA = PROGRAMASOCIAL.ID) WHERE ID_PERSONA = $id_persona";
	...
	function familiasRandom($num_familias){
		$conn = conexion();
